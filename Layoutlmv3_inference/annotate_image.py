...
def annotate_image(image_path, annotation_object, output_dir):
    img = None
    ...
    for prediction, box in zip(predictions, boxes):
        draw.rectangle(box, outline=label2color[prediction], width=3, fill=label2color[prediction]+(int(255*0.33),))
    ...
    output_path = os.path.join(output_dir, f'{image_name}_inference.jpg')
